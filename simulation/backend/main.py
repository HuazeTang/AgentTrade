...
class OrderBook:
    ...
    def _settle(self, buy: OrderIn, sell: OrderIn, price: float, qty: float, passive_price: float) -> Trade:
    ...
    def _match(self, incoming: OrderIn) -> List[Trade]:
        ...
        if incoming.side == "buy":
            while (
                ...
                passive_price = best.order.price
                trade_price = passive_price
                trades.append(self._settle(incoming, best.order, trade_price, match_qty, passive_price))
            ...
            while (
                ...
                passive_price = best.order.price
                trade_price = passive_price
                trades.append(self._settle(best.order, incoming, trade_price, match_qty, passive_price))
                incoming.quantity -= match_qty
                best.order.quantity -= match_qty
                if best.order.quantity > 0:
    ...
    def reset(self) -> None:
        """Clear order book, trades, and ledger."""
        with self._lock:
            self._buys.clear()
            self._sells.clear()
            self._ledger.clear()
