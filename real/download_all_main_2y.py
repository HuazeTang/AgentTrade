"""Download all main-board A-share data for past 2 years (2024-05-18 ~ 2026-05-18).

Usage: python download_all_main_2y.py
"""
...
# Save stock listing info
info_dir = Path("data/cache")
info_dir.mkdir(parents=True, exist_ok=True)
info_path = info_dir / "stock_list.parquet"
all_stocks.to_parquet(info_path, index=False)
print(f"\n股票列表已保存: {info_path} ({len(all_stocks)} 只)")

# Filter: main board, non-ST
main = all_stocks[(all_stocks["board"] == "main_board") & (~all_stocks["is_st"])]
syms = sorted(main["symbol"].tolist())
print(f"主板非ST: {len(syms)} 只")
...
for i in range(0, len(to_fetch), BATCH_SIZE):
    ...
    print(f"\n--- 批次 {batch_num}/{total_batches}: {len(batch)} 只 "
          f"({i+1}-{min(i+BATCH_SIZE, len(to_fetch))}/{len(to_fetch)}) ---")
    t0 = time.time()
    ...
    if total_elapsed > 0 and total_done > 0:
        ...
        print(f"  耗时: {elapsed:.0f}s | 累计: {total_elapsed/60:.1f}min | "
              f"进度: {completed}/{len(syms)} ({pct:.1f}%) | 失败: {total_failed} | "
