"""V4 Training: same architecture/loss as V3, raw OHLCV features + per-stock z-score."""
...
from dl_v4.dataset import YaoguDatasetV4, build_dataloaders_v4
from dl_v4.derived_features import ALL_V4_COLUMNS, build_v4_feature_cache
...
def train_v4(
    ...
    *,
    ...
    """Train DualTowerModel with V4 features (raw OHLCV + per-stock z-score).

    Returns (trained_model, training_metrics).
    """
    feature_cols = [c for c in ALL_V4_COLUMNS]
    if feature_cache is not None:
        # Include both cs and ts features
        feature_cols = [c for c in feature_cache.columns if not c.startswith("_")]

    logger.info("=" * 50)
    logger.info("Training V4: Raw OHLCV + Per-Stock Z-Score")
    ...
    logger.info("  Seq len: %d | Features: V4 (%d cols)",
                 sequence_length, len(feature_cols))
    logger.info("=" * 50)

    if feature_cache is None:
        logger.info("Building shared V4 feature cache...")
        feature_cache = build_v4_feature_cache(daily_cache)
        feature_cols = [c for c in feature_cache.columns if not c.startswith("_")]

    train_ds, val_ds, train_loader, val_loader = build_dataloaders_v4(
    ...
    for epoch in range(1, epochs + 1):
        ...
        ep_path = ckpt_dir / f"yaogu_v4_{run_id}_ep{epoch}.pt"
        checkpoint = {
            "epoch": epoch,
            "run_id": run_id,
        ...
        if val_ap > best_val_ap:
            ...
            best_epoch = epoch
            no_improve = 0
            best_path = ckpt_dir / f"yaogu_v4_{run_id}_best.pt"
            torch.save(checkpoint, best_path)
            logger.info("  → Best (val_ap=%.4f) → %s", val_ap, f"yaogu_v4_{run_id}_best.pt")
        else:
            no_improve += 1
