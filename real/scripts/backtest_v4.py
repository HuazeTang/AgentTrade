"""V4 Backtest: raw OHLCV + per-stock z-score, top-3, trail/hard/MA20 exit + model override.
...
CHECKPOINT = "data/models/yaogu_v4_20260530_1738_best.pt"

# ── Exit flags (for到期-only test, set both to False) ──
ENABLE_GAP_STOP = True
ENABLE_OTHER_EXITS = True  # hard stop, MA20, trailing stop
...
cache = build_v4_feature_cache(daily)
feature_cols = [c for c in cache.columns if not c.startswith("_")]
...
for i, idx in enumerate(test_dates):
    ...
    # ── Step 2: Gap-down stop (configurable) ──
    gap_stops = []
    if ENABLE_GAP_STOP:
        for sym_idx, pos in list(positions.items()):
            if pos.pending_sell:
    ...
    for sym_idx, pos in list(positions.items()):
        ...
        reason = None
        if ENABLE_OTHER_EXITS and (px - pos.cost) / pos.cost <= HARD_STOP:
            reason = "硬止损"
        elif ENABLE_OTHER_EXITS and ma_break:
            reason = "放量破MA20"
        elif ENABLE_OTHER_EXITS and pos.peak_high > 0 and (px - pos.peak_high) / pos.peak_high <= -TRAILING_STOP:
            reason = "移动止盈"
...
print(f"\n{'='*70}")
print(f"V4 Backtest — {BACKTEST_START.year}-{BACKTEST_END.year}")
print(f"Features: raw OHLCV + per-stock z-score ({len(feature_cols)} dims)")
